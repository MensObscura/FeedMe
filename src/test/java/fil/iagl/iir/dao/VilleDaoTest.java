...
public class VilleDaoTest extends AbstractDaoTest {
	...
	public void getAllTestSucces() throws Exception {
		List<Ville> villes = this.villeDao.getAll();

		Assertions.assertThat(villes).hasSize(NB_VILLE).have(new Condition<Ville>() {
			@Override
			public boolean matches(Ville ville) {
				return ville.getId() != null && ville.getCp() != null && ville.getNom() != null
						&& ville.getPays() != null;
			}
		});
	}
